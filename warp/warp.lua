--------------------------------------------------------------------------------
-- Teleportation network client. Run by user to initiate a teleport.
-- 
...
We cannot take cells out of the spatial IO port input slot if there is a problem (the user will need to do this instead).

Warp process (see `warp_daemon.lua` for the receiving side):
