...
function dlog.checkArgs(...)
  ...
  for i = 2, arg.n, 2 do
    if not string.find(arg[i], type(arg[i - 1]), 1, true) then
      error("bad argument at index #" .. i - 1 .. " (" .. arg[i] .. " expected, got " .. type(arg[i - 1]) .. ")")
    end
  end
end
...
function dlog.checkArgs2(...)
  for i = 2, select("#", ...), 2 do
    if not string.find((select(i, ...)), type(select(i - 1, ...)), 1, true) then
      error("bad argument at index #" .. i - 1 .. " (" .. (select(i, ...)) .. " expected, got " .. type(select(i - 1, ...)) .. ")")
    end
  end
end
...
function dlog.checkArgs3(v, t, ...)
  if not string.find(t, type(v), 1, true) then
    error("bad argument at index #" .. i - 1 .. " (" .. t .. " expected, got " .. type(v) .. ")")
  end
  if select("#", ...) > 0 then
...
function dlog.checkArgs4(val, typ, ...)
  if typ then
    if not string.find(typ, type(val), 1, true) then
      error("bad argument at index #" .. i - 1 .. " (" .. typ .. " expected, got " .. type(val) .. ")")
    end
...
function dlog.checkArgs5(val, typ, ...)
  if not string.find(typ, type(val), 1, true) then
    error("bad argument at index #1 (" .. typ .. " expected, got " .. type(val) .. ")", 3)
  end
...
checkArgsHelper = function(i, val, typ, ...)
  if typ then
    if not string.find(typ, type(val), 1, true) then
      error("bad argument at index #" .. i .. " (" .. typ .. " expected, got " .. type(val) .. ")", 3)
    end
